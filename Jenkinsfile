...
/*     when {
     ...
     steps {
       container('helm') {
         sh "helm upgrade --install ${NAME} https://github.com/mysticrenji/flask-mysql-k3s/tree/main/charts  --namespace ${NAME} --set image.repository=${IMAGE} --set image.tag=${VERSION}"
       }
     }
   }
